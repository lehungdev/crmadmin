 ...
 *
 * Most important Model of CrmAdmin which looks after Module, ModuleField Generation.
...
class Module extends Model
{
                ...
                foreach($fields as $field) {
                    ...
                    if(!isset($mod->id)) {
                        if($field->field_type == "Multiselect" || $field->field_type == "Taginput") {
                            ...
                            if(is_string($field->defaultvalue) && Str::of($field->defaultvalue)->startsWith('[')) {
        ...
        switch($field->field_type) {
            ...
            case 'Multiselect':
                ...
                if(is_array($field->defaultvalue)) {
                    ...
                    $var->default($field->defaultvalue);
                } else if(is_string($field->defaultvalue) && Str::of($field->defaultvalue)->startsWith('[')) {
                    $var->default($field->defaultvalue);
            ...
            case 'Text':
                ...
                if($field->maxlength == 0) {
                    ...
                    if($update) {
                        $var = $table->text($field->colname, $field->maxlength)->nullable()->change();
                    } else {
                        $var = $table->text($field->colname, $field->maxlength)->nullable();
                    }
                    if($field->defaultvalue != "") {
                        $var->default($field->defaultvalue);
                    } else if($field->required) {
            ...
            case 'LongText':
                ...
                if($field->maxlength == 0) {
                    ...
                    if($update) {
                        $var = $table->longtext($field->colname, $field->maxlength)->nullable()->change();
                    } else {
                        $var = $table->longtext($field->colname, $field->maxlength)->nullable();
                    }
                    if($field->defaultvalue != "") {
                        $var->default($field->defaultvalue);
                    } else if($field->required) {
    ...
    public static function getListingColumns($module_id_name, $isObjects = false)
    {
        ...
        foreach($listing_cols as $col) {
            if(Module::hasFieldAccess($module->id, $col['id']) || $module_id_name == "History_contacts") {
                if($isObjects) {
