...
class ModuleFields extends Model
{
    ...
    public static function createField($request)
    {
        ...
        if(!isset($field->id)) {
            ...
            //Langguege
            if($request->lang_active) {
                $field->lang_active = true;
            } else {
                $field->lang_active = false;
            }
            ...
            } else if(Schema::hasTable($module->name_db) && count($modulefields) == 0) { //dd($modulefields);
                // create SoftDeletes + Timestamps for module with existing table
                if(!Schema::hasColumn($module->name_db, 'created_at') and !Schema::hasColumn($module->name_db, 'deleted_at'))
        ...
        if(is_string($field->field_type) and !is_numeric($field->field_type)) {
...
//        else $field->defaultvalue = "";
    ...
    /**
     * Exclude the Columns form given list ($lang_active) if don't have field View Access
     * and return remaining Columns
     *
     * @param $module_name Module Name
     * @param $lang_active Array Listing Column Names
     * @return array Excluded array of Listing Column Names
     */
    public static function langColumnAccessScan($module_name, $lang_active)
    {
        $module = Module::get($module_name);
        $lang_active_temp = array();
        foreach($lang_active as $col) {
            if($col == 'id') {
                $lang_active_temp[] = $col;
            } else if(Module::hasFieldAccess($module->id, $module->fields[$col]['id'])) {
                $lang_active_temp[] = $col;
            }
        }
        return $lang_active_temp;
    }
}
