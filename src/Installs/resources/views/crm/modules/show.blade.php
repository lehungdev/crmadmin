...
<div id="page-content" class="profile2">
	...
	@endif
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<a class="text-white" href="{{ url(config('crmadmin.adminRoute') . '/'.$module->name_db) }}"><h4 data-toggle="tooltip" data-placement="left" title="Open {{ $module->model }} Module" class="name">{{ $module->label }}</h4></a>
					<div class="row stats">
	...
	<div class="tab-content">
						...
						<tbody>
							@foreach ($module->fields as $field)
								<tr>
									...
									<td>
										<form id="lang_active_cal" action="{{ url(config('crmadmin.adminRoute') . '/module_field_lang_active') }}">
									...
									<td style="min-width:60px;">
										...
										@if($field['colname'] != $module->view_col)
											<a href="{{ url(config('crmadmin.adminRoute') . '/modules/'.$module->id.'/set_view_col/'.$field['colname']) }}" class="btn btn-edit-field btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="view_col_{{ $field['colname'] }}"><i class="fa fa-eye"></i></a>
										@endif
									</td>
								</tr>
							@endforeach
		...
		<div role="tabpanel" class="tab-pane fade in p20 bg-white" id="tab-access">
			...
			<form action="{{ url(config('crmadmin.adminRoute') . '/save_role_module_permissions/'.$module->id) }}" method="post">
...
<div class="modal fade" id="AddFieldModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['route' => config('crmadmin.adminRoute') . '.module_fields.store', 'id' => 'field-form']) !!}
...
$(function () {
	...
	$('.delete_module').on("click", function () {
		...
		$.ajax({
			url: "{{ url(config('crmadmin.adminRoute') . '/get_module_files/') }}/" + module_id,
			type:"POST",
			beforeSend: function() {
	...
	function showValuesSection() {
		...
		if(ft_val == 2 || ft_val == 4 || ft_val == 5 || ft_val == 7 || ft_val == 9 || ft_val == 11 || ft_val == 12 || ft_val == 15 || ft_val == 18 || ft_val == 21 || ft_val == 24 || ft_val == 27 ) {
			$('#length_div').addClass("hide");
		}

		$('#unique_val').removeClass("hide");
		if(ft_val == 1 || ft_val == 2 || ft_val == 3 || ft_val == 7 || ft_val == 9 || ft_val == 11 || ft_val == 12 || ft_val == 15 || ft_val == 18 || ft_val == 20 || ft_val == 21 || ft_val == 24 || ft_val == 27 ) {
	...
	$("#generate_migr").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('crmadmin.adminRoute') . '/module_generate_migr') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
	...
	$("#generate_update").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('crmadmin.adminRoute') . '/module_generate_update') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$("#generate_migr_crud").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('crmadmin.adminRoute') . '/module_generate_migr_crud') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$('.Switch.Ajax.Lang').click(function() {
		...
		$.ajax({
			type: "POST",
			url : "{{ url(config('crmadmin.adminRoute') . '/module_field_lang_active') }}",
			data : {
				_token: '{{ csrf_token() }}',
