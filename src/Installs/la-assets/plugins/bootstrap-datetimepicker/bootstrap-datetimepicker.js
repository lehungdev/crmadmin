...
viewDate=parseInputDate(newDate);viewUpdate();return picker};if(element.is('input')){input=element}else{input=element.find(options.datepickerInput);if(input.size()===0){input=element.find('input[type=text]')}else if(!input.is('input')){throw new Error('CSS class "'+options.datepickerInput+'" cannot be applied to non input element')}}
if(element.hasClass('input-group')){if(element.find('.datepickerbutton').size()===0){component=element.find('.input-group-addon.input_dt')}else{component=element.find('.datepickerbutton')}}
