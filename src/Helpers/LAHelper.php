...
namespace Lehungdev\Crmadmin\Helpers;
...
/**
 * Class LAHelper
 * @package Lehungdev\Crmadmin\Helpers
 *
 * This is CrmAdmin Helper class contains methods required for Admin Panel functionality.
 */
class LAHelper
{
    /**
...
//        }
        ...
        $childrens = \Lehungdev\Crmadmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();
    ...
    public static function print_menu($menu, $active = false)
    {
        ...
        $str = '<li' . $treeview . ' ' . $active_str . '><a href="' . url(config("crmadmin.adminRoute") . '/' . $menu->url) . '"><i class="fa ' . $menu->icon . '"></i> <span>' . LAHelper::real_module_name($menu->name) . '</span> ' . $subviewSign . '</a>';
    ...
    public static function print_menu_topnav($menu, $active = false)
    {
        ...
        $str = '<li ' . $treeview . '' . $active_str . '><a ' . $treeview2 . ' href="' . url(config("crmadmin.adminRoute") . '/' . $menu->url) . '">' . LAHelper::real_module_name($menu->name) . $subviewSign . '</a>';
    ...
    /**
     * Get laravel version. very important in installation and handling Laravel 5.5 changes.
     *
